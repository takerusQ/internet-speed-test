...
# ハイパーパラメータ
input_dim = 512 * 512  # 512x512ピクセル
encoding_dim = 8 * 8  # エンコードされた表現の次元数を小さくする（例として64x64）
learning_rate = 0.01
epochs = 100  # エポック数を減らす
batch_size = 2  # バッチサイズを設定
...
LossとValidation Lossの違い
Loss（損失）は、トレーニングデータを使って計算されるモデルのパフォーマンス指標です。
トレーニング中の各バッチ処理で計算され、モデルのパラメータの更新に使用されます。損失が小さくなるように、モデルは学習していきます。

Validation Loss（バリデーション損失）は、バリデーションデータを使って計算されるモデルのパフォーマンス指標です。
バリデーションデータはトレーニングには使用されず、モデルの汎化性能を評価するために使われます。
エポックごとに計算され、トレーニング中のモデルが過学習やアンダーフィッティングしていないかを確認するために使用されます。
